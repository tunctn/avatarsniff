...
**Pipeline:** encoded bytes → `decode.ts` (→ RGBA pixels) → `analyze.ts` (→ verdict). Entry points in `bytes.ts` (`detectDefaultAvatar` / `detectDefaultAvatarFromUrl`) chain the two; `image-data.ts` (`detectFromImageData`) skips decode for a browser canvas `ImageData`. `index.ts` is the public surface — re-exports only.
...
**The heuristic (`analyze.ts`)** is purely *structural* — it never matches specific palette colours, so it keeps working as providers add new ones. A default = flat saturated background (not near-white, not near-black) + a small near-white glyph (the initial) + almost no other coloured content (rules out photos/illustrations). The image is downsampled (default 48×48), colours quantised, and a score combines dominance + simplicity. Every threshold is an overridable `DetectOptions` field (defaults in the `DEFAULTS` const); tune there, don't hardcode.
...
Tests in `lib/test/` generate **real image bytes in-test** via `test/image-helpers.ts` (in-house PNG/GIF encoders, `jpeg-js`, `@jsquash/webp`) — no production images are committed (privacy/licensing).

`test/fixtures.test.ts` is folder-driven: any image dropped into `test/fixtures/{default,real,local-default,local-real}/` becomes one test asserting its `isDefault` verdict. `default`/`real` are committed and public; `local-*` are git-ignored (`test/fixtures/local-*/`) for faces/PII and skip in CI. Empty folders skip. See `test/fixtures/README.md`.
